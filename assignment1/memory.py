...
class Memory:
    ...
    def sort_runs_blocks(self, prev_runs_dir_path, runs_dir_path, intermediate_runs_blocks_pointers):
        """
        this function is used to sort and merge runs, present in memory blocks
        """
        min_heap = []
        is_last_block = False
    ...
    def merge_runs(self, runs_dir_path: str, intermediate_runs_dir_path: str, intermediate_runs_counter: int):
        """
        this function merge the initial sorted runs and create intermediate sorted runs
        """
        ...
        while i < len(self.runs):
            # check for number of blocks in memory == (memory_size-1) as one memory block reserved for output block
        ...
        # when (#runs <= M-1)
        if self.size() > 0:
            # if memory blocks is not empty then sort the left blocks
            self.sort_runs_blocks(runs_dir_path, intermediate_runs_dir, intermediate_runs_blocks_pointers)
            self.memory_blocks.clear()
